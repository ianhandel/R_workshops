...
```{r}
alien_file <- here("data", "aliendata_20180331.xlsx")
...
Scatterplot - iron vs age - with fitted lines
```{r}
ggplot(alien) +
  aes(x = age, y = iron, colour = sex) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Alien iron vs age")
```

Then a simple linear model with age and sex as predictors...

```{r}
mod <- lm(iron ~ age + sex + region, data = alien)
```
