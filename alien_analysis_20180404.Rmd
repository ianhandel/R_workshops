...
As the data is stored in Excel we use the ```read_excel()``` function from the `readxl` packkage to import it. This function is given the filename, the sheet name or number (```sheet=```) and optionally the codes for missing data (```na=```). Our code reads the first sheet from "alien_analysis_20180404.Rmd" into a new R object called `alien`. Rmemeber that we're not changing the Excel sheet - just reading it into R.

```{r}
alien <- read_excel("aliendata_20180404.xlsx", sheet = 1, na = "-999")
```

Once the data is in R we can look at the top of it by clicking on its name in the "environment" tab/panel (top right part of the RSTudio window) or by typing ```View(alien)``` into the console (note that "it "View"" has a capital "V"). Once we've had a look we can strat cleaning and tidying it ready for analysis.

## Data tidy and clean

### Fill missing regions
First thing to do is to fill down the missing cells in the "region" column. The following R code takes the ```alien``` object, pipes it into the ```fill``` function/verb and fills the "region" column. The result is put back into the ```alien``` object with ```<-```. Remember, this changes the ```alien``` object in R but doesn't alter the spreadsheet. This is important. It means our original data is untouched as always available if we change out minds about our tidying!

If you want to look at what the code is doing one step at a time you can run each code chunk in the ```alien_analysis_20180404.Rmd``` file at a tie (using the little green arrow at its right hand side) and keep looking back at the ```alien``` data to see it change.  
